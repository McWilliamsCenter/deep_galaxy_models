...
def resnet_decoder(code,
    ...
    with tf.variable_scope(scope, [code], reuse=reuse) as sc:
        ...
        for i in range(1, num_stages+1):
            ...
            net = wide_resnet(net, current_depth, resample='up', activation_fn=activation, scope='resnet%d_a'%i)
            net = wide_resnet(net, current_depth, resample=None, activation_fn=activation, scope='resnet%d_b'%i)
...
def resnet_encoder(inputs,
    ...
    with tf.variable_scope(scope, [inputs], reuse=reuse) as sc:
        ...
        net = tf.layers.conv2d(inputs, base_depth, kernel_size=4, activation=activation, strides=2, name='conv2')

        for i in range(1, num_stages+1):
            print(i)
            current_depth = base_depth * 2**i
            net = wide_resnet(net, current_depth, resample=None, scope='resnet%d_a'%i)
            net = wide_resnet(net, current_depth, resample='down', scope='resnet%d_b'%i)
